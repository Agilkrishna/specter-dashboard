...
# Telegram Group: https://t.me/linux_repo
# Please read README.md file.

__version__ = 2.1 # Can change version from here
...
# Global variables initialized for API use
try:
...
def checkVersion():
	...
	if int(version) == 3:
		...
		sys.exit("\033[91m[!] Please use python version 3 ...\033[92m")
...
# Removed interactive loading logic for API stability
...
class searchInfoNumero:
	...
	def search(self, num):
		...
		for n in tags:
			line = n.text if n else ""
			p.append(line)

		operator = p[2] if len(p) > 2 else "Unknown"
		ville = p[3] if len(p) > 3 else "Unknown"
...
class facebookSearchTool:
	...
	def getInfoProfile(self, profile):
		...
		try:
			page = requests.get(url).content.decode('utf-8')
			findId_match = re.search(r"entity_id=([0-9]+)", page)
			
			if findId_match:
				facebookID = findId_match.group(1) # Use group 1 to get just the ID
			else:
				facebookID = "None"
		...
		name_match = re.search(r'pageTitle\">(.*)</title>', page)
			
		if name_match:
			name = name_match.group(1).replace(" | Facebook", "")
			self.name = name
...
def emailSpam():
	try:
		...
		try:
			...
			print("\033[92m[+] Connection completed successfully!\033[92m\n")
		except:
		...
		while True:
			...
			if s == "help":
				...
				try:
					...
					print(f"[+] Facebook Graph URL found: {facebookUrl % (facebookID)}")
				except ValueError:
					pass
		break
...
def Search118218():
	...
	for name in nameList:
		name = name.find("a")
		namesList2.append(name.text.strip() if name else "N/A")
	for addr in addresseList:
		adresse = addr.find("span")
		addressesList2.append(adresse.text.strip() if adresse else "N/A")
	for dep in depList:
		dep = dep.find("span")
		depList2.append(dep.text.strip() if dep else "N/A")
	for phone in phoneList:
		phoneList2.append(phone.text.strip() if phone else "N/A")
...
def searchCopainsdavant(nom, city):
	...
	for url in urlList:
		url = url.find("a")
		urls = str(url)
		href_match = re.search(r"/p/([a-zA-Z0-9_-]+)", urls)
		href = href_match.group(0) if href_match else "/p/unknown"
	...
	for name in nameList:
		...
		namesList2.append(name.text.strip() if name else "N/A")
	for addr in addresseList:
		addressesList2.append(addr.text.strip() if addr else "N/A")
...
def searchPersonne():
	nom = input("\n[#][LittleBrother][Lookup][Firstname name:~$ ")
	city = input("\n[#][LittleBrother][Lookup][Ville/Departement:~$ ")
	print(wait+" Research ...")
...
def google():
	nom = input("\n[#][LittleBrother][Lookup][name, first name, city (...):~$ ")
...
def doxMaker():
	...
	f.write("\nPostal Code: "+cp)
	f.write("\nIP Address: "+ip)
	s = input(" Do you want to add social networks to the list? [Y/N]: ").upper()
	if s == 'Y':
		while True:
	...
	if s == 'Y':
		print(" ATTENTION!  press ENTER when finished!\n")
...
def hashdecrypt():
	...
	if password:
		...
		print(warning+" %s : No match found." % (hash))
...
def showDataBase():
	...
	def searchProfiles():
		...
		if nameType == 'ID' :
			...
			if nameFile is None:
				print(warning+"\033[91m ID not found.\033[92m")
			else:
			...
			if not find:
				...
				if not find:
					print("\033[91m[!] No profiles found for\033[92m] '%s'." % (name))
...
"""
	...
	menu = "\n\033[92m[s] Search Profiles     [a] Show all Profiles    [e] Exit Database    [c] Clear screen   [h] Help message"
	...
	for data in datas:
		...
		if str(x) == '0':
			print("\033[91m[!] No profiles found.\033[92m")
		...
		while True:
			...
			if choix.lower() == 'h':
				...
				print("\033[91mCommand not found\033[92m")
...
def lb_header():
    ...
    headers = [header1, header2, header7, header9,
        header11, header12, header13, header14, header15, header16, header18, header19]
...
helpLookup = """\033[92m
 ...
 Phone lookup                     Do research with a phone number.
 ...
 Exit script                      Exit the software.
 Clear screen                     Clears Screen. Clears everything from Screen."""
...
lookupOption = """\033[92m
 ...
 [b] Back to main menu        [e] Exit script
 [h] Help Message             [c] Clear Screen"""

setoolOption = """\033[92m
 [1] SMS
 [2] Spam email
 [b] Back to main menu    [e] Exit script    [h] Help Message    [c] Clear Screen 
"""
...
` `._ ` _.'`"(     `-"'`._ ' _.' '           First coded by \033[95mLulz3xploit\033[92m 
     ~~~      `.          ~~~                Modified by \033[94mAbirHasan2005\033[92m
	...
	try:
		while True:
			...
			if choix.lower() == 'h':
				...
				sys.exit(information+" \033[92mJoin Telegram Group for feedback:\033[96m http://t.me/linux_repo\033[0m")
			elif choix == '1':
				clear()
				menu()
				print(lookupOption)
				while True:
					...
					#  ...
						...
						print("\033[91mCommand not found!\033[0m")
			elif choix == '2':
				clear()
				menu()
				print(setoolOption)
